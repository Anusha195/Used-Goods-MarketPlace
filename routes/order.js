...
router.post("/order/:id", async (req, res) => {
  ...
  try {
    ...
    if (!product) {
      return res.send(
        `<script>alert("Item not found."); window.location.href = "/dashboard";</script>`
      );
    }
...
router.get("/orders", async (req, res) => {
  ...
  try {
    ...
    const user = await User.findById(req.session.user._id);
    const ordersPlaced = await Order.find({ buyerId: userId })
      .populate("productId")
      .populate("sellerId");
    const ordersReceived = await Order.find({ sellerId: userId })
      .populate("productId")
      .populate("buyerId");
...
router.post("/cancel-order/:id", async (req, res) => {
  ...
  try {
    ...
    if (
      !order ||
      order.buyerId.toString() !== req.session.user._id.toString()
    ) {
    ...
    await order.save();
    res.send(
      `<script>alert("Order cancelled successfully!"); window.location.href = "/orders";</script>`
    ); // ✅ Option 2: Alert + redirect
  } catch (err) {
