...
router.get("/become-seller", async (req, res) => {
  ...
  if (user.isSeller) {
    return res.send(
      `<script>alert("You are already a seller."); window.location.href = "/dashboard";</script>`
    );
  }
...
router.post("/become-seller", async (req, res) => {
  ...
  await user.save();
  res.send(
    `<script>alert("Seller application submitted! Please wait for admin approval."); window.location.href = "/dashboard";</script>`
  );
});
