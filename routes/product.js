...
router.get("/postnew", async (req, res) => {
  if (!req.session.user) {
    return res.send(
      `<script>alert("Please login first."); window.location.href = "/";</script>`
    );
  }
  const user = await User.findById(req.session.user._id);
...
router.post("/post-item", upload.single("itemImage"), async (req, res) => {
  if (
    ...
    !req.session.user.sellerDetails.isApprovedByAdmin
  ) {
    return res.send(
      `<script>alert("You are not approved as seller yet!"); window.location.href = "/dashboard";</script>`
    );
  }
  ...
  try {
    ...
    res.send(
      `<script>alert("Item posted successfully!"); window.location.href = "/dashboard";</script>`
    );
  } catch (err) {
...
router.get("/mylistings", async (req, res) => {
  ...
  const user = await User.findById(req.session.user._id);
  if (!userId)
    return res.status(401).send("Please login first to view your listings.");
  try {
...
router.get("/edit/:id", async (req, res) => {
  try {
    ...
    if (!product)
      return res.send(
        `<script>alert("Product not found."); window.location.href = "/";</script>`
      );
    const user = await User.findById(req.session.user._id);
...
router.put("/edit/:id", async (req, res) => {
  ...
  res.send(
    `<script>alert("Updated successfully."); window.location.href = "/";</script>`
  );
...
router.post("/delete/:id", async (req, res) => {
  try {
    ...
    res.send(
      `<script>alert("Deleted successfully."); window.location.href = "/";</script>`
    );
    res.redirect("/mylistings");
  } catch (err) {
